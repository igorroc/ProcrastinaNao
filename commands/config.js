	...
	if (opcao1 == "comandos") {
		...
		if (opcao2 == "add") {
			...
			if (nome && tipo && id) {
				...
				try {
					...
					message.channel.send(
						`\\✅ Cargo \`${nome}\` adicionado com sucesso!`
					)
					console.log(`↳ Cargo '${nome}' adicionado com sucesso.`)
					message.guild.channels.cache
						.get("722274694535053317")
						.send(
							`\\▶ [LOGS] ⇥ Cargo \`${nome}\` adicionado com sucesso.`
						)
				} catch (error) {
					console.log(error)
					message.channel.send(
						`\\❌ Ocorreu um erro ao adicionar o cargo novo.`
					)
					console.log(`↳ Ocorreu um erro ao adicionar o cargo novo.`)
					message.guild.channels.cache
						.get("722274694535053317")
						.send(
							`\\▶ [LOGS] ⇥ Ocorreu um erro ao adicionar o cargo \`${nome}\`.`
						)
				}
			}
				...
				function readFileCallback(err, data) {
					if (err) {
						...
						if (achou) {
							...
							message.channel.send(
								`Cargo \`${cargo}\` removido com sucesso!`
							)
							message.guild.channels.cache
								.get("722274694535053317")
								.send(
									`\\▶ [LOGS] ⇥ Cargo \`${cargo}\` removido.`
								)
						} else {
				...
				function readFileCallback(err, data) {
					if (err) {
						...
						if (achouItem) {
							...
							console.log(
								`↳ Cargo '${cargo}', item '${itemParaSerMudado}' alterado para '${valor}'`
							)
							message.channel.send(
								`Cargo \`${cargo}\` alterado com sucesso!`
							)
							message.guild.channels.cache
								.get("722274694535053317")
								.send(
									`\\▶ [LOGS] ⇥ Cargo \`${cargo}\`, item \`${itemParaSerMudado}\` alterado para \`${valor}\``
								)
						} else if (achouCargo) {
							console.log(
								`↳ Chave '${itemParaSerMudado}' não encontrada`
							)
							message.channel.send(
								`Chave \`${itemParaSerMudado}\` não encontrada!`
							)
			...
			let achou = cargos.find(
				(c) =>
					c.name === exemplo || c.aliases.find((v) => v === exemplo)
			)
			...
			let embed = new Discord.MessageEmbed()
				...
				.setFooter(
					`Anti-Procrastinador | Cargo ${i + 1} de ${cargos.length}`,
				...
				.on("collect", async (reaction) => {
					...
					if (chosen === left) {
						...
						embed
							...
							.addField(
								"**Variações:**",
								cargos[i].aliases || "Sem variação"
							)
							.setFooter(
								`Anti-Procrastinador | Cargo ${i + 1} de ${
									cargos.length
								}`,
								bot.user.displayAvatarURL
							)
						...
						embed
							.addField(
								"**Nome:**",
								`${cargos[i].name || "Sem nome"}`
							)
							.addField("**ID:**", `${cargos[i].id || "Sem ID"}`)
							.addField(
								"**Tipo:**",
								`${cargos[i].type || "Sem tipo"}`
							)
							.addField(
								"**Variações:**",
								`${cargos[i].aliases || "Sem variação"}`
					...
					const userReactions = mCargos.reactions.cache.filter(
						(reaction) =>
		...
		if (config.status == opcao1) {
			...
			if (opcao1 == "on") {
				...
				message.guild.channels.cache
					.get("722274694535053317")
					.send(`\\▶ [LOGS] ⇥ ${on} STATUS: \`ONLINE\``)
				await bot.user.setActivity(
					`| .help para ajuda | Criado por Igor Rocha |`,
					{ type: "PLAYING" }
				)
				...
				message.guild.channels.cache
					.get("722274694535053317")
					.send(`\\▶ [LOGS] ⇥ ${off} STATUS: \`OFFLINE\``)
				await bot.user.setActivity(
					`| Estão fazendo alterações em mim! |`,
					{ type: "PLAYING" }
				)
		...
		message.channel.send(
			"Digite `comandos`, `cargos`, `emojis`, `on`, `off`"
		)
	}
}
