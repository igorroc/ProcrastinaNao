...
module.exports.run = async (bot, message, args) => {
    ...
    if(!member.kickable){
        ...
        console.log(`↳ Usuário "${args[0]}" não pode ser preso, operação cancelada.`)
        return
    }
    ...
    if(member.roles.cache.has('842189200337666058')){
        embed.setColor("#8915bf")
            ...
            .setTimestamp()
        member.roles.remove('842189200337666058').catch(() => console.log(`⚠️ Não foi possível remover o cargo para '${user.username}'`))
    }else{
