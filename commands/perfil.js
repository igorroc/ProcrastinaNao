	...
	if (args[0]) {
		if (args[0] == "all") {
			...
			let cEmbed = new Discord.MessageEmbed()
				...
				.addField(
					`Email:`,
					`[${perfil.email}](https://${perfil.email})`,
					false
				)
				.setFooter(
					`${indice + 1}/${total}`,
					...
					if (chosen === right) {
						...
						try {
							cEmbed = new Discord.MessageEmbed()
								...
								.addField(
									`Matrícula:`,
									`_${perfil.matricula}_`,
									true
								)
								.addField(
									`Ano de Egresso:`,
									`_${perfil.anoEgresso}_`,
									true
								)
								.addField(
									`Email:`,
						...
						msg.delete().catch(() =>
							console.log("↳ ⚠️ Erro ao deletar o embed")
						)
						message
							.delete()
							.catch(() =>
				...
				fs.readFile(
					...
					function readFileCallback(err, data) {
						if (err) {
							...
							console.log(
								`↳ Perfil de '${user.username}' removido`
							)
							...
							message.guild.channels.cache
								...
								.send(
									`\\▶ [LOGS] ⇥ Perfil de \` ${user.username} \` removido.`
								)
						}
					}
				)
			...
			if (perfil) {
				let cEmbed = new Discord.MessageEmbed()
					.setColor("#00ff00")
					.setTitle(
						`Perfil de ${message.mentions.members.first().nickname}`
					)
					.addField(`Nome:`, perfil.nome, false)
					.addField(`Matrícula:`, `_${perfil.matricula}_`, true)
					.addField(`Ano de Egresso:`, `_${perfil.anoEgresso}_`, true)
				...
				message.channel.send(
					`Perfil de \`${user.username}\` ainda não foi criado.`
				)
			}
		}
	} else {
		...
		if (perfil) {
			...
			console.log(
				`↳ '${message.author.username}' começou a fazer o perfil.`
			)
			message.guild.channels.cache
				.get("722274694535053317")
				.send(
					`\\▶ [LOGS] ⇥ \` ${message.author.username} \` começou a fazer o perfil.`
				)
			let cEmbed = new Discord.MessageEmbed()
				.setColor("#ff0000")
			...
			cEmbed
				.addField(
					`**Nome Completo:**  ${loading}`,
					`Olá ${message.member.user}, nos informe o seu nome completo`
				)
			...
			message.channel
				...
				.then(async (collected) => {
					...
					cEmbed
						.addField(
							`**Matrícula:**  ${loading}`,
							`${message.member.user}, nos informe seu número de matrícula\n> Exemplo: \`202010123\``
						)
					...
					message.channel
						...
						.then(async (collected) => {
							...
							cEmbed
								.addField(
									`**Email:**  ${loading}`,
									`${message.member.user}, informe seu email para contato.\n> Dê preferência o email institucional.\n> Exemplo: \`abc@uesc.br\``
								)
								.setFooter(
									`Passo 3 de 5`,
									bot.user.displayAvatarURL()
							...
							message.channel
								...
								.then(async (collected) => {
									...
									cEmbed
										.addField(
											`**Ano de Egresso:**  ${loading}`,
											`${message.member.user}, nos informe a previsão de término do curso.\n> Exemplo: \`2024\``
										)
										.setFooter(
											`Passo 4 de 5`,
											bot.user.displayAvatarURL()
									...
									message.channel
										...
										.then(async (collected) => {
											...
											cEmbed
												.addField(
													`**Foto de Perfil:**  ${loading}`,
													`${message.member.user}, envie a sua foto de perfil.\n> Basta arrastar a imagem para o chat, ou nos mandar o link da sua foto.`
												)
												.setFooter(
													`Passo 4 de 5`,
													bot.user.displayAvatarURL()
											...
											message.channel
													...
													fs.readFile(
														...
														function readFileCallback(
															...
															if (err) {
																...
																console.log(
																	`↳ Perfil de '${message.author.username}' criado com sucesso`
																)
																message.guild.channels.cache
																	.get(
																		"722274694535053317"
												...
												.catch((err) => {
													...
													console.log(err)
													message.channel.send(
														`Ocorreu um **erro** ao criar seu perfil, **tente novamente** mais tarde ou fale com alguém do <@&721329022621057074>`
													)
												})
										})
										.catch((err) => {
