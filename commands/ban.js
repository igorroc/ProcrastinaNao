...
module.exports.run = async (bot, message, args) => {
	...
	if (!member.bannable) {
		...
		console.log(
			`↳ Usuário "${args[0]}" não pode ser banido, operação cancelada.`
		)
		return
	}
	...
	if (!tempo || !Number.isInteger(tempo)) {
		return message.channel.send(
			"Indique um período valido para o banimento do usuário."
		)
	}

	if (!razao) {
		return message.channel.send(
			"Indique uma razão para o banimento do usuário."
		)
	}
